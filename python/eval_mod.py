...
write_path = os.path.join('/data/DeepMVS', dataset_name)
...
for (frame_idx, frame) in enumerate(sparse_model.image_list.images):
	...
	output_depth_raw = output_depth.copy()
	...
	d_mask = output_depth_raw > 0.0
	d = output_depth_raw

	d = np.where(d_mask, 1.0/d, 0.0)

	name = frame.filename
	name = name.split('.')[:-1]
	name = name[0]+"."+(name[1]).split('_')[0]+'_dpt.png'

	im = Image.fromarray((d * 5000.0).astype(np.uint16))
	im.save(os.path.join(write_path, name))
